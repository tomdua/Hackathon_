...
def start_tcp_game():
    ...
    # while end - start <= 5:
    #     try:
    #         client_keys[0].settimeout(0.1)
    #         char = client_keys[0].recv(1024).decode("utf-8")
    #         if(len(char) > 0):
    #             counter = counter + 1
    #         print(char)
    #     except:
    ...
    # val = str(counter)
    # val_b = val.encode()
    # client_keys[0].send(b'Game over!\nGroup 1 typed in ' + val_b)
    # client_keys[0].close()

    data = client_keys[0].recv(bufferSize)
    # print(number)
    ...
    #       tcp_con1=client_keys[1]
    #       char = tcp_con1.recv(bufferSize).decode('utf-8')
    #       score_group1+=char
    # for client_keys in group2.values():
    #     tcp_con1=client_keys[1]
    #     number = tcp_con1.recv(bufferSize).decode('utf-8)
    #     score_group2+=number
    ...
    #         score[name] += 1
    # socket.send(get_end_massage().encode())
    # socket.close()
    # num_of_threads.pop()
    # return
    # for client_keys in group1.values():
    #       readable, writable, errored = select.select([socket], [], [], 0)
    #       tcp_con=client_keys[1]
    #       if len(readable) > 0:
    #          char = tcp_con.recv(1).decode()
    #       score_group1+=char
    # for client_keys in group2.values():
    #     tcp_con=client_keys[1]
    #     number = len(tcp_con.recv(bufferSize).decode('utf-8'))
    #     score_group2+=number
